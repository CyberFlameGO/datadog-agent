...
// Some information, such as struct offsets, function parameter and return lists,
...
func (i *inspectionState) run() (*Result, error) {
	// First, go version and ABI to use in other stages of the inspection

	// Find the Go version and ABI for the binary
	// (and store it in the `i` struct).
	if i.dwarfInspectionState != nil {
		goVersion, abi, err := i.findGoVersionAndABIFromDWARF()
		if err != nil {
			return nil, err
		}
		i.abi = abi
		i.goVersion = goVersion
	} else {
		goVersion, abi, err := i.findGoVersionAndABIFromLinkerString()
		if err != nil {
			return nil, err
		}
		i.abi = abi
...
// findGoVersionAndABIFromDWARF attempts to determine the Go version and ABI
// from the debug symbols that are in the binary.
// It finds the first compile unit,
// and since all compile units must have been compiled by the same version of Go,
// takes its version as the version of the entire binary.
func (i *inspectionState) findGoVersionAndABIFromDWARF() (goversion.GoVersion, GoABI, error) {
	// Get the first compile unit in the DWARF metadata,
	// and grab its producer attribute
	if len(i.dwarfInspectionState.compileUnits.All) == 0 {
		return goversion.GoVersion{}, "", fmt.Errorf("no compile unit DWARF info entries found in binary; could not determine Go version and ABI")
	}

	firstCompileUnit := i.dwarfInspectionState.compileUnits.All[0]
	if producer, ok := firstCompileUnit.Entry.Val(dwarf.AttrProducer).(string); ok {
		// Remove the segment after ";" if it exists
		beginning := strings.Split(producer, ";")
		goVersion := goversion.ParseProducer(beginning[0])
		abi := determineABIFromProducer(producer)
		return goVersion, abi, nil
	}

	return goversion.GoVersion{}, "", fmt.Errorf("compile unit DWARF info entry lacked producer attribute; could not determine Go version and ABI")
}

// determineABIFromProducer determines the ABI from the producer string (in the debug info)
func determineABIFromProducer(producer string) GoABI {
	if strings.Contains(producer, "regabi") {
		return GoABIRegister
	}

	return GoABIStack
}

// findGoVersionAndABIFromLinkerMagic attempts to determine the Go version
...
func (i *inspectionState) findGoVersionAndABIFromLinkerString() (goversion.GoVersion, GoABI, error) {
	version, err := binversion.GetElfVersion(i.elfFile)
	if err != nil {
	...
	var abi GoABI
	if i.arch == GoArchX86_64 && parsed.AfterOrEqual(goversion.GoVersion{Major: 1, Minor: 17}) {
		abi = GoABIRegister
	} else {
		abi = GoABIStack
	}
...
func (i *inspectionState) inspectFunctionUsingDWARF(entry *dwarf.Entry, config FunctionConfig) (FunctionMetadata, error) {
	...
	for _, formalParamEntry := range formalParameterEntries {
		...
		if isReturn {
			// Only parse return parameters if explicitly enabled.
			// This might not produce useful location information,
			// especially if optimizations were enabled.
			// In such cases, a manual register allocation/stack layout
			// can be derived for the return values.
			if config.IncludeReturn {
				returnParameter, err := i.getParameterLocationAtPC(formalParamEntry, lowPC)
				if err != nil {
					paramName, _ := formalParamEntry.Val(dwarf.AttrName).(string)
					return FunctionMetadata{}, fmt.Errorf("could not inspect return value %q on function %q: %w", paramName, config.Name, err)
				}

				returns = append(returns, returnParameter)
			}
		} else {
	...
	return FunctionMetadata{
		...
		EntryLocation:   lowPC,
		Parameters:      params,
		IncludesReturn:  config.IncludeReturn,
		Returns:         returns,
