...
func GetWriteParams(version goversion.GoVersion, goarch string) ([]bininspect.ParameterMetadata, error) {
	...
	case "amd64":
		...
		return nil, fmt.Errorf("unsupported version go%d.%d.%d (min supported: go%d.%d.%d)", version.Major, version.Minor, version.Rev, 1, 15, 0)
	default:
...
// GetReadParams gets the return parameter metadata (positions/types) for crypto/tls.(*Conn).Read
func GetReadReturns(version goversion.GoVersion, goarch string) ([]bininspect.ParameterMetadata, error) {
	switch goarch {
	case "amd64":
		if version.AfterOrEqual(goversion.GoVersion{Major: 1, Minor: 15, Rev: 0}) {
			return []bininspect.ParameterMetadata{{TotalSize: 8, Kind: 0x2, Pieces: []bininspect.ParameterPiece{}}, {TotalSize: 16, Kind: 0x14, Pieces: []bininspect.ParameterPiece{}}}, nil
		}
		return nil, fmt.Errorf("unsupported version go%d.%d.%d (min supported: go%d.%d.%d)", version.Major, version.Minor, version.Rev, 1, 15, 0)
	default:
		return nil, fmt.Errorf("unsupported architecture %q", goarch)
