...
type DockerCheck struct {
	...
	containerAccessor generic.ContainerAccessor
...
func (d *DockerCheck) Configure(config, initConfig integration.Data, source string) error {
	...
	d.processor = generic.NewProcessor(metrics.GetProvider(), d.containerAccessor, metricsAdapter{}, getProcessorFilter(d.containerFilter))
	d.setOkExitCodes()
...
func (d *DockerCheck) Run() error {
	...
	return d.run(sender, du)
}

func (d *DockerCheck) run(sender aggregator.Sender, du docker.Client) error {
	rawContainerList, err := du.RawContainerList(context.TODO(), dockerTypes.ContainerListOptions{All: true})
	if err != nil {
		sender.ServiceCheck(DockerServiceUp, coreMetrics.ServiceCheckCritical, "", nil, err.Error())
	...
	if err := d.processor.Run(sender, d.Interval()/2); err != nil {
		return err
	}

	// Custom part on top of generic check
...
func (d *DockerCheck) runDockerCustom(sender aggregator.Sender, du docker.Client, rawContainerList []dockerTypes.Container) error {
	...
	for _, rawContainer := range rawContainerList {
		...
		isContainerExcluded := d.containerFilter.IsExcluded(rawContainer.Names[0], resolvedImageName, rawContainer.Labels["io.kubernetes.pod.namespace"])
		...
		if rawContainer.SizeRw > 0 {
			sender.Gauge("docker.container.size_rw", float64(rawContainer.SizeRw), "", containerTags)
		}

		if rawContainer.SizeRootFs > 0 {
			sender.Gauge("docker.container.size_rootfs", float64(rawContainer.SizeRw), "", containerTags)
		}
	}
...
func (d *DockerCheck) collectImageMetrics(sender aggregator.Sender, du docker.Client, containersPerImage map[string]*containerPerImage) error {
	...
	if err != nil {
		...
		d.Warnf("Unable to list Docker images, err: %w", err)
		sender.ServiceCheck(DockerServiceUp, coreMetrics.ServiceCheckCritical, "", nil, err.Error())
		return err
	}
